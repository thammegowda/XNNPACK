...
static void init_hardware_config(void) {
  ...
  const bool use_x86_avx512amx = XNN_ENABLE_AVX512AMX && XNN_ARCH_X86_64 && use_x86_avx512vnnigfni && cpuinfo_has_x86_amx_int8();
    const bool use_x86_amx_bf16 =
    #if defined(XNN_ENABLE_F32_REDUCED) && XNN_ENABLE_F32_REDUCED
      XNN_ENABLE_AVX512AMX && XNN_ARCH_X86_64 && use_x86_avx512skx && cpuinfo_has_x86_amx_bf16();
    #else
      false;
    #endif
  ...
  set_arch_flag(xnn_arch_x86_amx_bf16, use_x86_amx_bf16);
...
#if XNN_ARCH_X86_64 && defined(__linux__) && !defined(CHROMIUM)
  if (use_x86_avx512amx || use_x86_amx_bf16) {
    ...
    if (status) {
      ...
      set_arch_flag(xnn_arch_x86_amx_bf16, 0);
    }
  }
#endif
